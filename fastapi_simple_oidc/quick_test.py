...
login_item = '<p><img style="margin-bottom: -10px;margin-right: 10px" width="30" height="30" src="{logo}"/><a href="{url}">Login with <strong>{name}</strong></a></p>'
...
def test_page(providers, user, logout_url):
    if not user:
        content = '<div class="login-links">' + ''.join(
            login_item.format(url=provider.url, name=provider.name, logo=get_logo_url(provider.provider_url))
